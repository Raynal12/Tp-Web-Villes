...
public class CountryRepositoryTest {
    ...
    @Test
    @Sql("test-data.sql") // On peut charger des donnnées spécifiques pour un test
    void testPopulationPourLePays() {
        log.info("On rajoute une ville à la France, on doit trouver 12+4 = 16");
        
        Country paysQuiExisteDeja = new Country("XX", "France");
        City Castres = new City("Castres", paysQuiExisteDeja);
        Castres.setPopulation(4);
        
        int PopulationPourLePays = 12 + 4; // 12 dans data.sql, 4 dans test-data.sql
        
        assertEquals(PopulationPourLePays, 16, "On doit trouver 16 habitants" );
    }
